...
class Server{
	function __construct($config=array()){
		require(__DIR__ . '/../config/database.php');							//database config
		require(__DIR__ . '/../third_party/Oauth2/src/OAuth2/Autoloader.php');	//oauth library
...
//		$this->storage = new OAuth2\Storage\Pdo(array('dsn' => $config["dsn"], 'username' => $config["username"], 'password' => $config["password"]));
        ...
        OAuth2\Autoloader::register();
        $this->storage = new OAuth2\Storage\Pdo(array('dsn' => $config["dsn"], 'username' => $config["username"], 'password' => $config["password"]),
            array('user_table'=>'tbl_bookusers', 'get_user_filter'=>' AND isDeleted=0'));
...
//		$storage = new OAuth2\Storage\Memory(array('user_credentials' => $users));
    ...
    public function password_faceLogin($username){

//        $this->storage = new OAuth2\Storage\Pdo(array('dsn' => $config["dsn"], 'username' => $username, 'password' => "special"));
        $users = array($username => array("password" => 'special', 'dsn' => 'homeway', 'last_name' => 'yao'));
		$storage = new OAuth2\Storage\Memory(array('user_credentials' => $users));
